  ...
  module Model
    class Resource
      ...
      def attribute_text(path)
        ...
        case value
        when Array
          value.map { |item| item.is_a?(Hash) ? compact_hash(item) : Terradoc::Parser::ExpressionInspector.to_text(item) }
               .map(&:to_s)
               .join(", ")
        when Hash
      ...
      def attribute_ruby(path)
        value = attribute(path)
        return nil if value.nil?
        return value.map { |item| item.is_a?(Hash) ? hash_to_ruby(item) : Terradoc::Parser::ExpressionInspector.to_ruby(item) } if value.is_a?(Array)
